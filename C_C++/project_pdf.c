...
// Function to create PDF with multiple students in one file
void createMultipleStudentsPDF(Student *students, int count, const char *filename) {
    FILE *file = fopen(filename, "w");
    ...
    // Write transcripts for all students
    for (int s = 0; s < count; s++) {
        Student *student = &students[s];
        
        // Add student header if not the first student
        if (s > 0) {
            fprintf(file, "\n\n");
            fprintf(file, "===========================================================================\n");
            fprintf(file, "%32s STUDENT %d\n", "", s + 1);
            fprintf(file, "===========================================================================\n\n");
        }
    ...
    printf("\n========================================\n");
    printf("✓ All %d transcripts successfully saved to:\n", count);
    printf("  %s\n", filename);
    printf("========================================\n");
}

int main() {
    Student students[100];  // Array to store multiple students
    int studentCount = 0;
    char choice;
    ...
    while (continueAdding && studentCount < 100) {
        ...
        for (int i = 0; i < student->courseCount; i++) {
            ...
            while (!validMark) {
                ...
                if (student->courses[i].mark < 0 || student->courses[i].mark > 100) {
                    printf("❌ INVALID MARK! Mark must be between 0 and 100.\n");
                    printf("Please check the mark and enter again.\n");
                } else {
                    validMark = 1;
                }
            }
    ...
    if (studentCount == 0) {
        printf("ERROR: No students added!\n");
        return 1;
    }
    ...
    char filename[256];
    printf("\n========================================\n");
    printf("Enter filename to save transcripts\n");
    ...
    // Create full filename with .pdf extension
    char fullpath[300];
    sprintf(fullpath, "%s.pdf", filename);
    
    // Create PDF transcript with all students in one file
    createMultipleStudentsPDF(students, studentCount, fullpath);
